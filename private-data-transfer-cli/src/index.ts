...
async function main() {
  ...
  const sendData: FireFlyData[] = [{ value: "Hello" }, { value: "World" }];

  // Note: this is currently performing broadcast (not private transfer)
  // TODO: use private transfer
  console.log(
    `Broadcasting data values from firefly1: ${dataValues(sendData)}`
  );
  await firefly1.sendBroadcast(sendData);

  const receivedMessage = await ws2.firstMessageOfType(
    "message_confirmed",
    TIMEOUT
  );
  ...
  const transaction = await firefly2.getTransaction(
    receivedMessage.message.header.id
  );
  ...
  const receivedData = await firefly2.retrieveData(
